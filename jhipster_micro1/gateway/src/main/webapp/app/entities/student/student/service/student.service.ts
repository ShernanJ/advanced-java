...
export class StudentService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/students', 'student');
  ...
  protected convertDateFromClient(student: IStudent): IStudent {
    return Object.assign({}, student, {
      birthday: student.birthday?.isValid() ? student.birthday.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((student: IStudent) => {
        student.birthday = student.birthday ? dayjs(student.birthday) : undefined;
      });
    }
    return res;
  }
